...
def extract_pf_bc(element):
	f = open('table8.dat.txt', "r")
	for line in f:
		line = line.strip()
		column = line.split()
		if column[0]=="T[K]":
...
# Date: May 2019
# Author: Simon Grimm
# Modified by Stevanus K. Nugroho
# 30 May 2019: Using partition function from Barklem & Collet 2017
...
#		T, Q = np.loadtxt(pf_filename, unpack=True, usecols = (2,3), skiprows=3)
	if (j==0):
		element=elt0[i][1]+str('_I')
	elif(j==1):
		element=el[1][:-1]+str('_II')
	elif(j==2):
		element=el[1][:-1]+str('_III')
	print('')
	print (element)
	print('')
	T, Q = extract_pf_bc(element)
