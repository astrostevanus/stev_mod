...
def linestrength_normal(gf,nui,hatnu,QT0,T):
	...
	fac1= -h*c*nui/k/T
	fac2= -h*c*hatnu/k/T
	Snorm=fac0*gf*np.exp(fac1)*(-np.expm1(fac2)) #haven't normalised by the partition function
    
    #np.expm1(x) = exp(x) -1
    
	return Snorm

def linestrength_hitran_zero(gf,nui,hatnu,QT0,T0=296.):
...
def airWidth_Tdep_TiO_SharpsBurrows(jlist):
    #airWidth [cm-1] (Half width; HWHM) and Tdep by Sharps & Burrows (15)
    #jlist: numpy list of lower rotational quantum number J''
	w0=0.1
	w1=0.002
	gmairlist=0.5*(w0-w1*np.minimum(jlist,30))  #cm-1
	tdeplist=np.zeros(len(jlist))
    
	return gmairlist, tdeplist
